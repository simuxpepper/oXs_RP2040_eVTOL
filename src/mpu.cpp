...
bool MPU::getAccZWorld(){ // return true when a value is available ; read the IMU and calculate the acc on Z axis (world)
    ...
    // /* remove comment to disable yaw
    yaw   = RAD_TO_DEGREE * atan2(2.0 * (qh[1] * qh[2] + qh[0] * qh[3]), (qh[0] * qh[0] + qh[1] * qh[1] - qh[2] * qh[2] - qh[3] * qh[3]));
    ...
    // Yaw does not work as a sudden change in yaw will set that new angle as reference point.

    sent2Core0( CAMERA_PITCH_ID , (int32_t) (pitch * 10.0)) ;
    sent2Core0( CAMERA_ROLL_ID , (int32_t) (roll * 10.0)) ;
    //sent2Core0( CAMERA_YAW_ID , (int32_t) (yaw * 10.0)) ;
    ...
    if (now_ms - last_ms >= 500) {
        ...
        sent2Core0( YAW , (int32_t) yaw); //comment to remove yaw
